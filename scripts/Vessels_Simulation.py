...
class VascularSystem:
    def __init__(self, start_point, direction, initial_length, initial_radius, 
                 min_branching_angle=20, max_branching_angle=45, radius_ratio=0.7, length_ratio=0.8, 
                 max_depth=4, min_radius=0.1, random_seed=None, 
                 progressive_growth=False, visualization_interval=1.0):
        """
    ...
    def visualize_intersection(self, plane_params, intersections=None):
...
def main():
    ...
    vascular_system = VascularSystem(
        ...
        initial_length=10,
        initial_radius=2.0,
        min_branching_angle=20,  # Minimum branching angle
        max_branching_angle=45,  # Maximum branching angle
        radius_ratio=0.7,
        length_ratio=0.8,
        max_depth=7,  # Increased depth for more bifurcations
        min_radius=0.1,  # Smaller minimum radius to allow more bifurcations
        random_seed=seed,
        progressive_growth=True,  # Show growth process
        visualization_interval=0.5  # Time between visualizations
    )
    ...
    # Visualize the vascular system and the intersections
    fig = vascular_system.visualize_intersection(plane_params, intersections)
    plt.show()
